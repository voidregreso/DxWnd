// CDAudio1.cpp: implementation of the CCDAudio class.
//
//////////////////////////////////////////////////////////////////////
...
CCDAudio::CCDAudio() {
    m_nErrorCode = 0;
    m_nErrorCode = mciSendString( _T("open cdaudio"), NULL, 0, NULL );
}
...
MCIERROR CCDAudio::Pause() {
    TCHAR szBuff[128];
    memset(szBuff, 0, sizeof(szBuff));
    m_nErrorCode = mciSendString( _T("status cdaudio mode wait"), szBuff, sizeof(szBuff), NULL );
    if( m_nErrorCode != 0 )
        return false;
    if( lstrcmp( szBuff, _T("paused") ) == 0 )
        m_nErrorCode = mciSendString( _T("resume cdaudio"), NULL, 0, NULL );
    else
...
//=============================================================================
bool CCDAudio::SetVolume(unsigned int nVolume) {
    const int nBuffSize = 64;
    TCHAR szBuff[nBuffSize];
    TCHAR szReq[nBuffSize];
    if(nVolume > 100) nVolume = 100;
    _stprintf( szReq, _T("setaudio cdaudio volume to %d"), nVolume );
    m_nErrorCode = mciSendString( szReq, szBuff, nBuffSize, NULL );
    if( m_nErrorCode != 0 )
        return false;
    return true;
}
