...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved) {
    ...
    case DLL_PROCESS_ATTACH: {
        int fnamelen = strlen("\\d3d9.dll");
        CopyMemory(path + GetSystemDirectory(path, MAX_PATH - (fnamelen + 1)), "\\d3d9.dll", fnamelen + 1); // copy +1 byte for string terminator
        d3d9.dll = LoadLibrary(path);
        if (d3d9.dll == false) {
            char msg[512 + 1];
            sprintf_s(msg, 512, "Cannot load original d3d9.dll library\nerror=%d\npath=\"%s\"", GetLastError(), path);
            MessageBox(0, msg, "d3d9 proxy", MB_ICONERROR);
            ExitProcess(0);
        }
