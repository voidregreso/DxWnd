...
BOOL WINAPI DllMain(HINSTANCE hInst, DWORD reason, LPVOID) {
    if (reason == DLL_PROCESS_ATTACH) {
        char path[MAX_PATH];
        int fnamelen = strlen("\\winmm.dll");
        CopyMemory(path + GetSystemDirectory(path, MAX_PATH - (fnamelen + 1)), "\\winmm.dll", fnamelen + 1); // copy +1 byte for string terminator
        hLThis = hInst;
        hL = LoadLibrary(path);
        if (!hL) {
            char msg[512 + 1];
            sprintf_s(msg, 512, "Cannot load original winmm.dll library\nerror=%d\npath=\"%s\"", GetLastError(), path);
            MessageBox(0, msg, "winmm proxy", MB_ICONERROR);
            ExitProcess(0);
        }
