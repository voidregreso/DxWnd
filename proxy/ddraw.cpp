...
HRESULT WINAPI VoidSetAppCompatData(DWORD p1, DWORD p2) {
    return S_OK;
}
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved) {
    ...
    case DLL_PROCESS_ATTACH: {
        int fnamelen = strlen("\\ddraw.dll");
        CopyMemory(path + GetSystemDirectory(path, MAX_PATH - (fnamelen + 1)), "\\ddraw.dll", fnamelen + 1); // copy +1 byte for string terminator
        ddraw.dll = LoadLibrary(path);
        if (ddraw.dll == false) {
            char msg[512 + 1];
            sprintf_s(msg, 512, "Cannot load original ddraw.dll library\nerror=%d\npath=\"%s\"", GetLastError(), path);
            MessageBox(0, msg, "ddraw proxy", MB_ICONERROR);
            ExitProcess(0);
        }
        ...
        if(ddraw.SetAppCompatData == NULL)
            ddraw.SetAppCompatData = (FARPROC)VoidSetAppCompatData;
        InitDxWnd("ddraw");
        break;
    }
