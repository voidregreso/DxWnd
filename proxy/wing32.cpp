...
__declspec(naked) void FakeWinGCreateDC()					{
    _asm { jmp [wing32.WinGCreateDC] }
}
__declspec(naked) void FakeWinGCreateBitmap()				{
    _asm { jmp [wing32.WinGCreateBitmap] }
}
__declspec(naked) void FakeWinGBitBlt()						{
    _asm { jmp [wing32.WinGBitBlt] }
}
__declspec(naked) void FakeWinGStretchBlt()					{
    _asm { jmp [wing32.WinGStretchBlt] }
}
__declspec(naked) void FakeWinGRecommendDIBFormat()			{
    _asm { jmp [wing32.WinGRecommendDIBFormat] }
}
__declspec(naked) void FakeWinGGetDIBPointer()				{
    _asm { jmp [wing32.WinGGetDIBPointer] }
}
__declspec(naked) void FakeWinGSetDIBColorTable()			{
    _asm { jmp [wing32.WinGSetDIBColorTable] }
}
__declspec(naked) void FakeWinGGetDIBColorTable()			{
    _asm { jmp [wing32.WinGGetDIBColorTable] }
}
__declspec(naked) void FakeWinGCreateHalftonePalette()		{
    _asm { jmp [wing32.WinGCreateHalftonePalette] }
}
__declspec(naked) void FakeWinGCreateHalftoneBrush()		{
    _asm { jmp [wing32.WinGCreateHalftoneBrush] }
}
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved) {
    ...
    case DLL_PROCESS_ATTACH: {
        int fnamelen = strlen("\\wing32.dll");
        CopyMemory(path + GetSystemDirectory(path, MAX_PATH - 10), "\\wing32.dll", fnamelen + 1); // copy +1 byte for string terminator
        wing32.dll = LoadLibrary(path);
        if (wing32.dll == false) {
            char msg[512 + 1];
            sprintf_s(msg, 512, "Cannot load original wing32.dll library\nerror=%d\npath=\"%s\"", GetLastError(), path);
            MessageBox(0, msg, "wing32 proxy", MB_ICONERROR);
            ExitProcess(0);
        }
