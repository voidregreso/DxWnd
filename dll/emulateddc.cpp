...
HDC dxwCore::AcquireEmulatedDC(HWND hwnd) {
    ...
    OutDebugDW("AcquireEmulatedDC: hwnd=%#x Desktop=%#x WinRect=(%d,%d)(%d,%d) VirtRect=(%d,%d)(%d,%d)\n",
    ...
    _if(!VirtualHDC) OutTraceE("AcquireEmulatedDC: CreateCompatibleDC ERROR err=%d at=%d\n", GetLastError(), __LINE__);
    VirtualPic = CreateCompatibleBitmap(wdc, dxw.GetScreenWidth(), dxw.GetScreenHeight());
    _if(!VirtualPic) OutTraceE("AcquireEmulatedDC: CreateCompatibleBitmap ERROR err=%d at=%d\n", GetLastError(), __LINE__);
    if(!(PrevSelection = (*pSelectObject)(VirtualHDC, VirtualPic)))
        ...
        _if(!ret) OutTraceE("AcquireEmulatedDC: DeleteObject ERROR pic=%#x err=%d at=%d\n", PrevSelection, GetLastError(), __LINE__);;
    }
    ...
    _if(!ret) OutTraceE("AcquireEmulatedDC: StretchBlt ERROR err=%d at=%d\n", GetLastError(), __LINE__);
    ret = (*pGDIReleaseDC)(hwnd, wdc);
    _if(!ret) OutTraceE("AcquireEmulatedDC: ReleaseDC ERROR hdc=%#x err=%d at=%d\n", wdc, GetLastError(), __LINE__);
    return VirtualHDC;
}
...
BOOL dxwCore::ReleaseEmulatedDC(HWND hwnd) {
    ...
    (*pGetClientRect)(hwnd, &WinRect);
    OutDebugDW("ReleaseEmulatedDC: hwnd=%#x Desktop=%#x WinRect=(%d,%d)(%d,%d) VirtRect=(%d,%d)(%d,%d)\n",
               hwnd, dxw.IsDesktop(hwnd),
               WinRect.left, WinRect.top, WinRect.right, WinRect.bottom,
               VirtualPicRect.left, VirtualPicRect.top, VirtualPicRect.right, VirtualPicRect.bottom);
    if(!(wdc = (*pGDIGetDC)(hwnd))) { // potential DC leakage
    ...
    _if(!ret) OutTraceE("ReleaseEmulatedDC: StretchBlt ERROR err=%d at=%d\n", GetLastError(), __LINE__);
    ret = (*pGDIReleaseDC)(hwnd, wdc); // fixed DC leakage
    _if(!ret) OutTraceE("ReleaseEmulatedDC: ReleaseDC ERROR hwnd=%#x hdc=%#x err=%d at=%d\n", hwnd, wdc, GetLastError(), __LINE__);
    return TRUE;
}
