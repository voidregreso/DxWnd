...
void DumpDibSection(char *api, HDC hdc, const BITMAPINFO *pbmi, UINT iUsage, VOID *pvBits) {
    static int prog = 0;
    FILE *fdump;
    char path[81];
    ...
    if((iUsage != DIB_RGB_COLORS) && (iUsage != DIB_PAL_COLORS)) return;
    if(!pvBits) return;
    if((pbmi->bmiHeader.biWidth == 1) && (pbmi->bmiHeader.biHeight == 1)) return; // don't dump just built 1x1 DIBs
    ...
    OutTrace("DumpDibSection: api=%s prog=%08.8d size=%d wxh=(%dx%d) usage=%s bc=%d sizeimg=%d planes=%d comp=%#x ppm=(%dx%d) colors=%d imp=%d\n",
             api, prog,
...
void DumpHDC(HDC hdc, int x0, int y0, int w, int h) {
    ...
    OutTrace("DumpDevContext: prog=%08.8d hdc=%#x size=%d wxh=(%dx%d) bc=%d sizeimg=%d planes=%d comp=%#x ppm=(%dx%d) colors=%d imp=%d\n",
             prog, hdc,
