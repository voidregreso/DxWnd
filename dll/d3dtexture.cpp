...
void D3DTextureDump(D3DSURFACE_DESC Desc, D3DLOCKED_RECT LockedRect) {
    ...
    while (TRUE) { // fake loop 1
        ...
        if(!TextureSize(Desc, LockedRect)) {
            OutTraceDW("TextureDump: UNSUPPORTED\n");
            break;
        }
        // calculate the bitmap hash
        hash = D3DHash(Desc, LockedRect);
        if(!hash) {
            OutTraceDW("TextureDump: hash=NULL\n");
            break; // almost certainly, an empty black surface!
        }
        OutDebugDW("TextureDump: hash=%08.8X size=(%dx%d) format=%#x(%s)\n",
                   hash, Desc.Width, Desc.Height, Desc.Format, ExplainD3DSurfaceFormat(Desc.Format)
                  );
        // Create the file.
        ...
        case FORMAT_RAW:
            ...
            if(fwrite((BYTE *)LockedRect.pBits, TextureSize(Desc, LockedRect), 1, hf) != 1)
                OutTraceE("TextureDump: fwrite ERROR err=%d\n", GetLastError());
#else
...
void D3DTextureHack(D3DSURFACE_DESC Desc, D3DLOCKED_RECT LockedRect) {
    ...
    while(TRUE) { // fake loop
        ...
        h = Desc.Height;
        if((MinTexX && (w < MinTexX)) || (MinTexY && (h < MinTexY))) {
            OutTraceDW("TextureHack: SKIP small texture\n");
            break;
        }
        if((MaxTexX && (w > MaxTexX)) || (MaxTexY && (h > MaxTexY))) {
            OutTraceDW("TextureHack: SKIP big texture\n");
            break;
        }
        iSurfaceSize = Desc.Height * LockedRect.Pitch;
        // calculate the bitmap hash
        hash = D3DHash(Desc, LockedRect);
        if(!hash) {
            OutTraceDW("TextureHack: hash=NULL\n");
            break; // almost certainly, an empty black surface!
        }
        ...
        case FORMAT_BMP:
            ...
            if(iBitCount == 0) {
                OutTraceDW("TextureHack: unsupported format=%#x\n", Desc.Format);
                break;
            }
            ...
            while(TRUE) { // fake loop to ensure final fclose
                ...
                case D3DFMT_DXT1:
                    ...
                    if(pbi.bV4Height < 0) {
                        ...
                        if(!fb) {
                            OutTraceE("TextureHack: malloc error\n");
                            break;
                        }
                        fseek(hf, hdr.bfOffBits, SEEK_SET);
                        if(fread((LPVOID)fb, Desc.Height * Desc.Width * (iBitCount / 8), 1, hf) != 1) {
                            OutTraceE("TextureHack: fread error\n");
                            free(fb);
                            break;
                        }
                        // biHeight < 0 -> scan lines from top to bottom, same as surface/texture convention
                        for(int y = 0; y < (int)Desc.Height; y += 4) {
                            for(int x = 0; x < (int)Desc.Width; x += 4) {
                                OutTraceDW("Compressing line=%d row=%d\n", y, x);
                                stb_compress_dxt_block(p, fb, FALSE, STB_DXT_NORMAL);
                                p += (16 * sizeof(DWORD) / 8);
                                OutTraceDW("Compression done\n");
                                fb += 4 * 2;
                            }
                        }
                        free(fb);
...
void D3DTextureHighlight(D3DSURFACE_DESC Desc, D3DLOCKED_RECT LockedRect) {
    OutDebugDW("TextureHighlight: size=(%dx%d) format=%d(%s)\n",
               Desc.Width, Desc.Height, Desc.Format, ExplainD3DSurfaceFormat(Desc.Format)
              );
    DWORD mask16 = 0x0;
    switch (Desc.Format) {
    case D3DFMT_X8R8G8B8:
