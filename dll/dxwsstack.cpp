...
void dxwSStack::ClearSurfaceList() {
    ...
    for (i = 0; i < DDSQLEN; i++) {
        ...
        case SURFACE_ROLE_PRIMARY:
            lpDDSPrimary = SurfaceDB[i].lpdds;
            break;
        case SURFACE_ROLE_BACKBUFFER:
            lpDDSBackBuffer = SurfaceDB[i].lpdds;
            break;
        case SURFACE_ROLE_ZBUFFER:
            lpDDSZBuffer = SurfaceDB[i].lpdds;
            break;
        case SURFACE_ROLE_3DREF:
            lpDDS3DRef = SurfaceDB[i].lpdds;
            break;
        }
    }
...
void dxwSStack::PushSurface(LPDIRECTDRAWSURFACE ps, USHORT role, USHORT version, DWORD dwCaps) {
    ...
    for (i = 0; i < DDSQLEN; i++) {
        ...
        if (iRoleKount == MAXROLEKOUNT) iMax = i; // v2.04.82: set index to last matching entry
    ...
    switch(e->uRole) {
    case SURFACE_ROLE_PRIMARY:
        lpDDSPrimary = e->lpdds;
        break;
    case SURFACE_ROLE_BACKBUFFER:
        lpDDSBackBuffer = e->lpdds;
        break;
    case SURFACE_ROLE_ZBUFFER:
        lpDDSZBuffer = e->lpdds;
        break;
    case SURFACE_ROLE_3DREF:
        lpDDS3DRef = e->lpdds;
        break;
