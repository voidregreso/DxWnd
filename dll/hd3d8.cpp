...
void D3D8TextureHandling(void *arg, int Level) {
    ...
    OutDebugD3D("D3D8TextureHandling: arg=%#x level=%d type=%d(%s)\n", (DWORD)arg, Level, TexType, sTexType(TexType));
    ...
    OutDebugD3D("D3D8TextureHandling: level=%d type=%#x usage=%#x\n", Level, Desc.Type, Desc.Usage);
    switch(Desc.Type) {
    case D3DRTYPE_SURFACE:
    case D3DRTYPE_TEXTURE:
        break;
    default:
        OutDebugD3D("D3D88TextureHandling: SKIP type=%#x\n", Desc.Type);
        return;
        break;
    }
    if(Desc.Usage == D3DUSAGE_RENDERTARGET) {
        OutDebugD3D("D3D8TextureHandling: SKIP usage=%#x\n", Desc.Usage);
        return;
    }
    //pSurfaceLevel->GetRenderTargetData(&pRenderTarget, &pDestSurface);
    res = (*pLockRect8)(lpd3dtex, Level, &LockedRect, NULL, 0);
    OutTraceDW("D3D8TextureHandling: lpd3dtex=%#x level=%d format=%#x(%s) size=(%dx%d) bits=%#x pitch=%d\n",
...
HRESULT  dxCopyRects(void *lpd3dd, LPDIRECTDRAWSURFACE psrc, LPDIRECTDRAWSURFACE pdst) {
    ...
    if(res = lpsrc->GetDesc(&SrcDesc)) {
        ...
        OutDebugD3D("dxCopyRects: source size=(%dx%d) format=%d\n", SrcDesc.Width, SrcDesc.Height, SrcDesc.Format);
    ...
    if(res = lpdst->GetDesc(&DstDesc)) {
        ...
        OutDebugD3D("dxCopyRects: dest size=(%dx%d) format=%d\n", DstDesc.Width, DstDesc.Height, DstDesc.Format);
    ...
    case D3DFMT_X8R8G8B8:
        ...
        case D3DFMT_R5G6B5: {
            OutDebugD3D("dxCopyRects: converting 32 to 16 BPP\n", res, __LINE__);
            DWORD *srcpix;
            WORD *dstpix;
        ...
        case D3DFMT_A8R8G8B8:
        case D3DFMT_X8R8G8B8: {
            OutDebugD3D("dxCopyRects: converting 32 to 32 BPP\n", res, __LINE__);
            DWORD *srcpix;
            DWORD *dstpix;
            for(unsigned int y = 0; y < min(DstDesc.Height, SrcDesc.Height); y++) {
                srcpix = (DWORD *)SrcLockedRect.pBits + (y * SrcLockedRect.Pitch / 4);
    ...
    case D3DFMT_R5G6B5:
        ...
        case D3DFMT_A8R8G8B8:
        case D3DFMT_X8R8G8B8: {
            OutDebugD3D("dxCopyRects: converting 16 to 32 BPP\n", res, __LINE__);
            WORD *srcpix;
            DWORD *dstpix;
        ...
        break;
        case D3DFMT_R5G6B5: {
            OutDebugD3D("dxCopyRects: converting 16 to 16 BPP\n", res, __LINE__);
            WORD *srcpix;
            WORD *dstpix;
            for(unsigned int y = 0; y < min(DstDesc.Height, SrcDesc.Height); y++) {
                srcpix = (WORD *)SrcLockedRect.pBits + (y * SrcLockedRect.Pitch / 2);
        ...
        break;
    default:
        OutTraceE("dxCopyRects: UNSUPPORTED format src=%#x dst=%#x\n", SrcDesc.Format, DstDesc.Format);
    }
...
#ifndef DXW_NOTRACES
    if(res) OutTraceE("%s: CreateRenderTarget ERROR res=%#x at %d\n", api, res, __LINE__);
#endif // DXW_NOTRACES
...
#ifndef DXW_NOTRACES
    if(res) OutTraceE("%s: SetRenderTarget ERROR res=%#x at %d\n", api, res, __LINE__);
#endif // DXW_NOTRACES
    OutTraceD3D("%s: OK\n", api);
...
HRESULT dwD3D8Present(void *lpd3ddx, CONST RECT *pSourceRect, CONST RECT *pDestRect, HWND hDestWindowOverride, CONST RGNDATA *pDirtyRegion) {
    HRESULT res;
    LPDIRECT3DDEVICE8 lpd3dd = (LPDIRECT3DDEVICE8)lpd3ddx;
    if (hDestWindowOverride || pDirtyRegion) {
        OutTraceE("dwD3D8Present: ERROR only parameter-less Present is supported\n");
        return D3DERR_INVALIDCALL;
    }
    if (pSourceRect || pDestRect) {
        OutTraceE("dwD3D8Present: WARNING only full surfaces are supported\n");
        pSourceRect = NULL;
        pDestRect = NULL;
    }
