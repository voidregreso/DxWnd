...
DWORD WINAPI CpuSlow(LPVOID lpThreadParameter) {
    ...
    OutTraceDW("starting CPUSlow dwOwnerPID=%#x Ratio=1:%d %s\n", dwOwnerPID, iSlowDownRatio, (dxw.dwFlags10 & PRECISETIMING) ? "PRECISE" : "SLEEP");
...
DWORD WINAPI CpuLimit(LPVOID lpThreadParameter) {
    ...
    DWORD dwOwnerPID = GetCurrentProcessId();
    DWORD dwOwnerThread = GetCurrentThreadId();
    OutTraceDW("starting CPULimit dwOwnerPID=%#x Ratio=1:%d %s\n", dwOwnerPID, iSlowDownRatio, (dxw.dwFlags10 & PRECISETIMING) ? "PRECISE" : "SLEEP");
    pNap = (dxw.dwFlags10 & PRECISETIMING) ? Nap : Sleep;
    if(!dwOwnerPID) return FALSE;
    if(iSlowDownRatio < 1) return FALSE;
...
BOOL SlowCpuSpeed(DWORD dwOwnerPID, DWORD dwOwnerThread, int iSlowDownRatio) {
    ...
    if(hThreadSnap == INVALID_HANDLE_VALUE) {
        OutTraceDW("LimitCpuUsage: CreateToolhelp32Snapshot ERROR err=%d\n", GetLastError());
        return FALSE;
    }
    ...
    if(!Thread32First(hThreadSnap, &te32)) {
        OutTraceDW("Thread32First ERROR: err=%d\n", GetLastError());  // Show cause of failure
...
static FILETIME FTFTSUM(FILETIME a, FILETIME b) {
    ...
    __try {
        s.dwLowDateTime = a.dwLowDateTime + b.dwLowDateTime;
    } __except (EXCEPTION_EXECUTE_HANDLER) {
        s.dwHighDateTime += 1;
    }; // add reminder on overflow
    return s;
}
...
static FILETIME FTDWSUM(FILETIME a, DWORD b) {
    ...
    __try {
        s.dwLowDateTime = a.dwLowDateTime + b;
    } __except (EXCEPTION_EXECUTE_HANDLER) {
        s.dwHighDateTime += 1;
    }; // add reminder on overflow
    return s;
...
static BOOL RefreshThreadList(int *iNumThreads, ThreadDesc_Type ProcessThreads[], DWORD dwOwnerPID, DWORD dwOwnerThread) {
    ...
    OutTraceDW("LimitCpuUsage: refreshing thread list\n");
    te32.dwSize = sizeof(THREADENTRY32);
    // Take a snapshot of all running threads
    hThreadSnap = CreateToolhelp32Snapshot( TH32CS_SNAPTHREAD, 0 );
    if(hThreadSnap == INVALID_HANDLE_VALUE) {
        OutTraceE("LimitCpuUsage: CreateToolhelp32Snapshot ERROR err=%d\n", GetLastError());
        return FALSE;
    }
    // Retrieve information about the first thread, and exit if unsuccessful
    if(!Thread32First(hThreadSnap, &te32)) {
        OutTraceE("LimitCpuUsage: Thread32First ERROR err=%d\n", GetLastError());  // Show cause of failure
        CloseHandle(hThreadSnap);     // Must clean up the snapshot object!
        return FALSE;
    }
...
BOOL LimitCpuUsage(DWORD dwOwnerPID, DWORD dwOwnerThread, int iSlowDownRatio) {
    ...
    FILETIME CreationTime, ExitTime, KernelTime, UserTime;
    OutTraceDW("LimitCpuUsage: TimeLimit=%ld TimeSlot=%ld\n", iTimeLimit, iTimeSlot);
    // Fill in the size of the structure before using it.
