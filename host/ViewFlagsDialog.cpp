...
BOOL CViewFlagsDialog::OnInitDialog() {
    ...
    sflags.Append("\nFlags9: ");
    for(i = 0, dword = t->flags9; i < 32; i++, dword >>= 1) if(dword & 0x1) sflags.AppendFormat("%s ", GetFlagCaption(8, i));
    sflags.Append("\nFlags10: ");
    for(i = 0, dword = t->flags10; i < 32; i++, dword >>= 1) if(dword & 0x1) sflags.AppendFormat("%s ", GetFlagCaption(9, i));
    sflags.Append("\nFlags11: ");
    for(i = 0, dword = t->flags11; i < 32; i++, dword >>= 1) if(dword & 0x1) sflags.AppendFormat("%s ", GetFlagCaption(10, i));
    sflags.Append("\nFlags12: ");
    for(i = 0, dword = t->flags12; i < 32; i++, dword >>= 1) if(dword & 0x1) sflags.AppendFormat("%s ", GetFlagCaption(11, i));
    sflags.Append("\nTFlags: ");
    for(i = 0, dword = t->tflags; i < 32; i++, dword >>= 1) if(dword & 0x1) sflags.AppendFormat("%s ", GetFlagCaption(12, i));
