...
#define USAGE "Usage: bchunk [-v] [-r] [-p (PSX)] [-w (wav)] [-s (swabaudio)]\n" \
	...
	"  -i  ignoredata: do not extract data track\n"
...
#define READBINARYMODE "rb"
#define WRITEBINARYMODE "wb"
#else
#include <inttypes.h>
#include <netinet/in.h>
#define READBINARYMODE "r"
#define WRITEBINARYMODE "w"
#endif
...
void parse_args(int argc, char *argv[]) {
    int s;
    int argind;
    while ((s = getopt(argc, argv, "swivp?hr")) != -1) {
        switch (s) {
        case 'r':
            raw = 1;
            break;
    ...
    basefile = "Track";
    while (argind < argc) {
        switch (argind - optind) {
        //case 3:
        //	binfile = strdup(argv[optind]);
        //	break;
        case 0:
            cuefile = strdup(argv[argind]);
            break;
        case 1:
            basefile = strdup(argv[argind]);
            break;
        default:
            fprintf(stderr, "%s", USAGE);
            exit(1);
...
int writetrack(FILE *bf, struct track_t *track, char *bname) {
    ...
    if (!(f = fopen(fname, WRITEBINARYMODE))) {
...
int main(int argc, char **argv) {
    ...
    /* gho update: we care, get bin path from here */
    if (!fgets(s, CUELLEN, cuef)) {
        fprintf(stderr, "Could not read first line from %s: %s\n", cuefile, strerror(errno));
        return 3;
    }
    binfile = strdup(cuefile);
    p = binfile + strlen(binfile) - 4;
    if(strstr(s, ".bin")) strcpy(p, ".bin");
    if(strstr(s, ".BIN")) strcpy(p, ".bin");
    if(strstr(s, ".img")) strcpy(p, ".img");
    if(strstr(s, ".IMG")) strcpy(p, ".img");
    printf("data file: %s\n", binfile);
    if (!((binf = fopen(binfile, READBINARYMODE)))) {
    ...
    for (track = tracks; (track); track = track->next) {
        if(ignoredata && !track->audio) continue;
