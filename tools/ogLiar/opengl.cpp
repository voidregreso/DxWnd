...
BOOL WINAPI DllMain(HINSTANCE hInst, DWORD reason, LPVOID) {
    if (reason == DLL_PROCESS_ATTACH) {
        char path[MAX_PATH];
        int fnamelen = strlen("\\opengl32.dll");
        CopyMemory(path + GetSystemDirectory(path, MAX_PATH - (fnamelen + 1)), "\\opengl32.dll", fnamelen + 1); // copy +1 byte for string terminator
        hLThis = hInst;
        hL = LoadLibrary(path);
        if (!hL) {
            char msg[512 + 1];
            sprintf_s(msg, 512, "Cannot load original opengl32.dll library\nerror=%d\npath=\"%s\"", GetLastError(), path);
            MessageBox(0, msg, "opengl32 proxy", MB_ICONERROR);
            ExitProcess(0);
        }
...
// glGetString
//extern "C" __declspec(naked) void __stdcall __E__124__() { __asm {jmp p[124*4] }}
// glGetTexEnvfv
...
const GLubyte *WINAPI extglGetString(GLenum name) {
    if(name == GL_EXTENSIONS)
        return (const GLubyte *)"GL_EXT_LIE";
    typedef const GLubyte * (WINAPI * glGetString_Type)(GLenum);
    return (const GLubyte *)(glGetString_Type)&p[124 * 4];
}
